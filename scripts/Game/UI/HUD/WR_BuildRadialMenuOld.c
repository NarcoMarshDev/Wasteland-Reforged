...
class WR_BuildRadialMenuComponentClass : SCR_RadialMenuComponentClass
{
}

class WR_BuildRadialMenuComponent : SCR_RadialMenuComponent
{
	//------------------------------------------------------------------------------------------------
	...
	override void EOnFrame(IEntity owner, float timeSlice)
	{
		...
		{
			if (!canOpen || GetGame().GetInputManager().GetActionTriggered("CharacterMelee"))
			{
		...
		if (weapon)
		{
			...
			{
				WR_BuildMenuEntry entry = new WR_BuildMenuEntry(controlledCharacter, weapon, magazine);

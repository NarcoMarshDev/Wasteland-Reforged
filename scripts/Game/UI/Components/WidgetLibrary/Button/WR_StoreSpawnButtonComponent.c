...
class WR_StoreSpawnButtonComponent: SCR_NavigationButtonComponent
{
	...
	override bool OnClick(Widget w, int x, int y, int button)
	{
		...
		// if we do have enough money, do the correct purchase process depending on the store type
		
		// ----------------------------------------- THIS IS DIFFERENT FOR DIFFERENT STORE TYPES ----------------------------------------- //
		
		// #WR_TODO	- Add switch case for the different store types
		/**
		switch (m_CurrentStoreComponent.GetStoreType())
		{
			// ------------------------------------------------------------------------------------------------------------------------------- //	
			...
			// ------------------------------------------------------------------------------------------------------------------------------- //	
				...
				// set spawn slot as occupied
				spawnSlot.SetOccupant(spawnedEntity);
				// item has spawned, so take the money now and close the menu
				m_LocalPlayerProfile.m_MoneyCash -= m_CurrentEntry.m_iPrice;
				
				// #WR_TODO - Find out if this is necessary or if the reference to the player profile is enough for the money to update
				gmComponent.UpdatePlayerProfile(m_LocalPlayerProfile);
				GetGame().GetMenuManager().GetTopMenu().Close();
				break;
			// ------------------------------------------------------------------------------------------------------------------------------- //	
			...
			// ------------------------------------------------------------------------------------------------------------------------------- //	
			default:
				Print("Unknown store type!", LogLevel.ERROR);
				break;
		}
		*/
		return false;
